describe('A Live Event without user point related is correctly displayed on the timeline ', () => {
  ...
  before(() => {
    // Login
    cy.login()
    cy.url({ timeout: 10 * second }).should("contain", Cypress.config().baseUrl +"/#/v2/o/1/campaigns/display").then(() => {
      cy.request({
        method:  "post", 
        url: Cypress.env('apiDomain') + "/v1/datamarts/1162/user_activities?processing_pipeline=true&async=false",
        body: {
          "$type":"SITE_VISIT",
        ...
        cy.wait(500);
        cy.visit(Cypress.config().baseUrl + '/#/v2/o/504/audience/timeline/user_agent_id/vec:10?datamartId=1162')
      })
    })
  })
  ...
  it('Should display json source modal', () => {
    ...
    cy.get('.ant-timeline-item').eq(1).find('.mcs-card-title').first().should('have.text', 'Site: mediarithmics')
