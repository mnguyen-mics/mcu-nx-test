...
it('should test the cleaning rules update form', () => {
    ...
    cy.readFile('cypress/fixtures/init_infos.json').then((data) => {
        ...
        // Add cleaning rule
        cy.contains('New Cleaning Rule',{timeout:10000}).parent().click({force:true})
...
it('should test that only DRAFT cleaning rules can be deleted and updated',()=>{
    cy.readFile('cypress/fixtures/init_infos.json').then((data) => {
        ...
        cy.get(`[href="#/v2/o/${data.organisationId}/settings/datamart/audience/partitions"]`).click()
        cy.get(`[href="#/v2/o/${data.organisationId}/settings/datamart/cleaning_rules"]`).click()
        // cy.visit(`${Cypress.config().baseUrl}/#/v2/o/${data.organisationId}/settings/datamart/cleaning_rules`,{timeout:10000})
        ...
        cy.get('button').contains('New Cleaning Rule').parent().click({force:true})
...
it('should check that we can only have 3 different life durations for user event cleaning rules with content filters',()=>{
    cy.readFile('cypress/fixtures/init_infos.json').then((data) => {
        ...
        cy.get(`[href="#/v2/o/${data.organisationId}/settings/datamart/cleaning_rules"]`).click()
        // cy.visit(`${Cypress.config().baseUrl}/#/v2/o/${data.organisationId}/settings/datamart/cleaning_rules`)
        cy.contains('New Cleaning Rule').parent().click({force:true})
