...
const createDisplayCreative = (
  ...
  return CreativeService.createDisplayCreative(organisationId, resource).then(
    (newCreative: DataResponse<DisplayAdResource>) => {
      const creativeId = newCreative.data.id;
      return Promise.all([
        ...properties.map(item =>
          CreativeService.updateDisplayCreativeRendererProperty(
            organisationId,
            creativeId,
            item.technical_name,
            item,
...
const updateDisplayCreative = (
  ...
  return CreativeService.updateDisplayCreative(creative.id, creative)
    .then(() => {
      ...
      return Promise.all(propertiesPromises)
        .then(() => {
          return CreativeService.takeScreenshot(creativeId)
            .then(() => {
              return creativeId;
            })
            .catch((err: any) => {
...
export { createDisplayCreative, updateDisplayCreative };
