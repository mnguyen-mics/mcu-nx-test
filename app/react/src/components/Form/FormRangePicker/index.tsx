...
class FormRangePicker extends React.Component<
  FormRangePickerProps & FormFieldWrapperProps,
  {}
> {
  ...
  disabledDate = (isStart: boolean) => (currentDate: moment.Moment) => {
    ...
    if (currentDate.isBefore(moment.now())) {
      return true;
    }
  ...
  render() {
    ...
    const error =
      (!startDate &&
        this.displayError(startProps.name) &&
        syncErrors &&
        syncErrors[startProps.name]) ||
      (!endDate &&
        this.displayError(endProps.name) &&
        syncErrors &&
        syncErrors[endProps.name]) ||
      '';
...
export default connect((state, ownProps: { formId: string }) => ({
