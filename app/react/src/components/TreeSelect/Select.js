...
function loopTreeData(data, level = 0) {
  return data.map((item, index) => {
    ...
    const props = {
      ...
      selectable: selectable === true ? true : selectable,
      ...otherProps,
    };
    let ret;
...
class Select extends Component {
  ...
  onSelect = (selectedKeys, info) => {
    ...
    if (checkEvt) {
      ...
      // if inputValue existing (and not hideSelected), tree is checkStrictly
  ...
  renderTopControlNode() {
    ...
    if (isMultipleOrTags(props) && !props.hideSelected) {
      ...
      >
        ...
        >
          ...
          this.state.value[0].value ? clear : null}
            ...
            {multiple ?
              this.getSearchPlaceholderElement(!!this.state.inputValue || (this.state.value.length && !this.props.hideSelected)) :
              null}
          </span>
        </span>
      </SelectTrigger>
