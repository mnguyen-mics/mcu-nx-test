    ...
    const renderPicker = (that) => {
      ...
      return (
        <Row>
          ...
          <Col span={12}>
            <Radio.Group defaultValue={options[1].key} onChange={e => onChangeValueRight(e, 1)}>
              {legends.map(legend => {
                return (
                  <Radio key={legend.key} style={radioStyle} value={legend.key}>
                    {legend.domain}
        ...
        {options.map(option => {
          return (
            <div key={option.domain} style={{ float: 'left' }}>
              ...
              />
              <span style={{ float: 'right', lineHeight: '40px', marginLeft: '5px' }}>{option.domain}</span>
            </div>
          );
        })}
        <div style={{ float: 'left', marginLeft: '10px', borderLeft: '1px solid #f0f0f0', height: '40px', lineHeight: '40px' }}>
          <button
          ...
          >
            <McsIcons style={{ marginLeft: '10px', color: '#d0d0d0' }} type="pen" />
          </button>
        </div>
      </div>
    );
