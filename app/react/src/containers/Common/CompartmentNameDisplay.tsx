...
export default class CompartmentNameDisplay extends React.Component<
  CompartmentNameDisplayProps,
  State
> {
  ...
  componentDidMount() {
    const { userAccountCompartmentId } = this.props;
  ...
  componentDidUpdate(previousProps: CompartmentNameDisplayProps) {
    ...
    const { userAccountCompartmentId: previousUserAccountCompartmentId } = previousProps;
  ...
  fetchUserAccountCompartment = (userAccountCompartmentId: string) => {
    ...
    return this._compartmentService
      .getCompartment(userAccountCompartmentId)
      .then(res => res.data)
      .then(res =>
        this.setState(
          { loading: false, compartment: res },
    ...
    return this.state.compartment ? (
      <span>
        {this.state.compartment.name
          ? this.state.compartment.name
          : this.state.compartment.token
          ? this.state.compartment.token
          : this.state.compartment.id}
      </span>
    ) : (
