...
class AvailableNodeVisualizer extends React.Component<{},State> {
  ...
  createGrid = (nodes: FakeNode[]) : JSX.Element[] => {
    return nodes.map(node =>{
      return <AvailableNode key={node.id} title={node.name} icon={node.icon} color={node.color}/>
    })
  }
  ...
  render() {
    ...
    <div> 
      <Row>
        ...
        <div className="available-node-grid">  {this.createGrid(this.state.actionNodes)} </div>
      </Row>  
      <Row className="available-node-visualizer-row">
        <div className="available-node-category-header"> Conditions </div>  
        <div className="available-node-grid">  {this.createGrid(this.state.conditionNodes)} </div>
      </Row>  
      <Row className="available-node-visualizer-row">
        <div className="available-node-category-header"> Exits </div>  
        <div className="available-node-grid">  {this.createGrid(this.state.exitsNodes)} </div>
      </Row>
    </div>
    )}
}
