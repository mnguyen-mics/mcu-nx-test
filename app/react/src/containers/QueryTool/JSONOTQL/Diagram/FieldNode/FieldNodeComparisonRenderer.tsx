...
type ComparisonOperator = StringComparisonOperator |
  TimeComparisonOperator |
  EnumComparisonOperator |
  BooleanComparisonOperator |
  NumericComparisonOperator;
...
const messages: {[key in ComparisonOperator]: FormattedMessage.MessageDescriptor } = defineMessages({
  EQ: {
...
export default class FieldNodeComparisonRenderer extends React.Component<Props> {
  ...
  render() {
    ...
    return (
      <div className="comparison">
        <div style={{
          color: node.getColor(),
        }} className="fieldname">{node.fieldNode.field}</div>
        <div><span className="operator">{operator ? <FormattedMessage {...messages[operator as ComparisonOperator]} /> : null}</span>{' '}{node.fieldNode.comparison && node.fieldNode.comparison.values ? this.renderValues(node.fieldNode.comparison) : null}</div>
      </div>
    );
  }
}
