...
function ResourceByKeywordSelector<T extends SelectableResource, AdditionalContext>(
  NameDisplay: React.ComponentType<T>,
  resourceFetcher: ResourceFetcher<T>,
  placeholder: string,
) {
  type Props = ResourceByKeywordSelectorProps & QueryFilter<AdditionalContext>;
  class Wrapped extends React.Component<
    Props & InjectedNotificationProps,
    ResourceByKeywordSelectorState
  > {
    private _debounce = debounce;
    constructor(props: Props & InjectedNotificationProps) {
      super(props);
      const value = props.value.map((key: string) => {
        return {
          key: key,
          value: key,
        };
    ...
    handleSearch = (keyword: string) => {
      if (keyword.length > 0) {
        ...
        if (
          ...
          fetchedKeyword &&
          keyword.toLowerCase().includes(fetchedKeyword.toLowerCase())
        )
          this.searchInPreviousFetch(keyword, fetchedResourcesList);
        else this.fetchListMethod(keyword);
      }
    };

    searchInPreviousFetch(keyword: string, fetchedResourcesList: LabeledValue[]) {
      this.setState({
        resourcesList: fetchedResourcesList.filter(
          labeledValue =>
            labeledValue.key && labeledValue.key.toLowerCase().includes(keyword.toLowerCase()),
        ),
      });
    }
    ...
    fetchListMethod(keyword: string) {
      ...
      return resourceFetcher
        .getForKeyword(options)
        .then(res => {
          const result: LabeledValue[] = res
            .filter(re => re.id)
            .map(r => ({ key: r.id, label: <NameDisplay {...r} showId={showId} />, value: r.id }));
          ...
          value.forEach(v => {
            ...
            if (!valueExistInList) {
              ...
              const formatedValue: LabeledValue = {
                key: v,
                label: <NameDisplay {...(unavailableValue as any)} />,
                value: v,
              };
    ...
    render() {
      ...
      return (
        ...
        >
          ...
          {resourcesList.map((item: LabeledValue, index: number) => (
            <Select.Option value={item.key as React.ReactText} key={index.toString()}>
              {item.label}
            </Select.Option>
          ))}
        </Select>
  ...
  return compose<Props & InjectedNotificationProps, Props>(injectNotifications)(Wrapped);
}
