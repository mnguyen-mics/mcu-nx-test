...
import { AudienceExternalFeedTyped, AudienceTagFeedTyped } from '../../Edit/domain';
...
export interface FeedCardProps {
  ...
  onFeedUpdate: (newFeed: AudienceExternalFeedTyped | AudienceTagFeedTyped) => void;
  onFeedDelete: (feed: AudienceExternalFeedTyped | AudienceTagFeedTyped) => void;
  segmentId: string;
...
const messages = defineMessages({
  ...
  modalDescription: {
    ...
    defaultMessage: 'Are you sure you want delete this feed ? Carefull this action cannot be undone.'
  },
  pause: {
...
class FeedCard extends React.Component<Props, FeedCardState> {
  ...
  renderActionButton = () => {
    ...
    switch (feed.status) {
      case 'ACTIVE':
        return <ButtonStyleless onClick={editFeed}>
  ...
  render() {
    ...
    const removeFeed = () => {
      ...
      const onOk = () => {
        if (feed.type === 'EXTERNAL_FEED') {
          return this.setState({ isLoading: true }, () => AudienceSegmentService.deleteAudienceExternalFeeds(segmentId, feed.id)
            .then(r => {
              this.setState({ isLoading: false })
              onFeedDelete(feed)
            })
        ...
        if (feed.type === 'TAG_FEED') {
          return this.setState({ isLoading: true }, () => AudienceSegmentService.deleteAudienceTagFeeds(segmentId, feed.id)
            .then(r => {
              this.setState({ isLoading: false })
              onFeedDelete(feed)
            })
