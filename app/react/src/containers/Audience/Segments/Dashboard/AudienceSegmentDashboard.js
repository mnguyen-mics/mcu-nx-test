...
class AudienceSegmentDashboard extends Component {
  ...
  render() {
    ...
    const headerItems = [{
      ...
      number: segment.report_view ? formatMetric(segment.report_view[0].user_points, '0,0') : '--'
    },
    {
      iconType: 'users',
      translationKey: 'USER_ACCOUNTS',
      number: segment.report_view ? formatMetric(segment.report_view[0].user_accounts, '0,0') : '--'
    },
    {
      iconType: 'display',
      translationKey: 'DISPLAY',
      number: segment.report_view ? formatMetric(segment.report_view[0].desktop_cookie_ids, '0,0') : '--'
    },
    {
      iconType: 'email-inverted',
      translationKey: 'EMAILS',
      number: segment.report_view ? formatMetric(segment.report_view[0].emails, '0,0') : '--'
    }];
...
AudienceSegmentDashboard = connect(
  mapStateToProps
)(AudienceSegmentDashboard);

AudienceSegmentDashboard = withRouter(AudienceSegmentDashboard);

export default AudienceSegmentDashboard;
