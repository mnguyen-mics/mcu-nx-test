...
import { AdInfoResource, DisplayCampaignInfoResource } from "../../../../../models/campaign/display";
...
export function generateCsvData(organsationId: string, campaign: DisplayCampaignInfoResource , macro: ExportType, ads: AdInfoResource[], formatMessage: (messageDescriptor: FormattedMessage.MessageDescriptor, values?: {[key: string]: MessageValue}) => string) {
  const data = ads.map(ad => {
    ...
    const adGroupId = campaign.ad_groups.find(adg => adg.ads.find(a => a.id === ad.id) ? true : false)!.id
    return {
      creative_name: ad.name,
      snippet_code: generateSnippet(ad.creative_id, Number(format[0]), Number(format[1]), macro, campaign.id, adGroupId)
    }
  })
