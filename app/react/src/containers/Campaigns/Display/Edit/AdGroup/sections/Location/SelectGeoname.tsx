...
interface State {
  ...
  incOrExc: string;
  listOfGeonamesToDisplay: Geoname[];
  country: string;
}
...
class SelectGeoname extends React.Component<JoinedProps, State> {
  ...
  // To Remove. Waiting backend part
  countries = [
  ...
  fetchGeonames = (value: string = '') => {
    ...
    // Waiting back end part. IF condition only for example
    if (country === 'France') {
      this.setState({ fetchingGeonames: true });
      // TODO use this.state.country to get related geonames
  ...
  render() {
    const { fetchingGeonames, listOfGeonamesToDisplay } = this.state;
        ...
        <div className="small-select small-select-right">
          <Select
            ...
            defaultValue={formatMessage(
              messages.contentSectionLocationOptionCountryPlaceholder,
            )}
