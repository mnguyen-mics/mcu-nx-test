...
class HistoryEventCard extends React.Component<Props, State> {
    ...
    this.props.events.forEach(event => {
      if (isHistoryLinkEvent(event)) {
        this.state.resourceNames[this.generateResourceIdentifier(event)] = (
          <FormattedMessage {...messages.fetchingData} />
        );
      }
    });
  }
      ...
      if (isHistoryLinkEvent(event)) {
        ...
        if (resourceHelper) {
          resourceHelper
            ...
            .catch(err => {
              this.setState(prevState => {
                prevState.resourceNames[this.generateResourceIdentifier(event)] = (
  ...
  renderValue = (field: string, value: string) => {
    ...
    return value ? (
      <span className='value'>
        {formatProperty(fieldToSnakeCase, value).formattedValue || value}
      </span>
    ) : (
  ...
  renderUniqueLinkEvent = (event: HistoryLinkEventResource) => {
    ...
    const messageChild = isHistoryCreateLinkEvent(event)
      ? messages.resourceCreateLinkChild
      : messages.resourceDeleteLinkChild;
    const messageParent = isHistoryCreateLinkEvent(event)
      ? messages.resourceCreateLinkParent
      : messages.resourceDeleteLinkParent;
    const goToResource = () => {
      resourceHelper.goToResource(event.resource_id);
    };

    return resourceHelper.direction === 'CHILD' ? (
      <FormattedMessage
        {...{
          ...messageChild,
          values: {
        ...
        {...{
          ...
          values: {
            ...
            childResourceType: (
              <span className='name'>
                <FormattedMessage
                  {...(formatProperty('history_resource_type').message ||
                    messages.defaultResourceType)}
                />
              </span>
            ),
  ...
  findCreateParentLinkEventIndex = (events: HistoryEventShape[]) => {
    ...
    return events.findIndex(event => {
      const rHelper =
        isHistoryCreateLinkEvent(event) &&
        resourceLinkHelper &&
        resourceLinkHelper[event.resource_type];
  ...
  render() {
    ...
    const createParentLinkEvent = events[
      createParentLinkEventIndex
    ] as HistoryCreateLinkEventResource;
    ...
    return (
      <Card>
          ...
          {events.length > 1 ? (
            <Row>
                  ...
                  <FormattedMessage
                    {...{
                      ...
                      values: {
                        ...
                        parentLink: createParentLinkEvent
                          ? this.renderLinkEventInMultiEdit(createParentLinkEvent, true)
                          : '',
                      },
                    }}
                  />
              ...
              <div className='section-cta'>
                <Button onClick={toggleDetails} className='mcs-card-inner-action'>
                  {!showMore ? (
                ...
                <div className='mcs-fields-list'>
                  {this.renderMultiEdit(
                    createParentLinkEventIndex > -1
                      ? [
                          ...events.slice(0, createParentLinkEventIndex),
                          ...events.slice(createParentLinkEventIndex + 1),
                        ]
                      : events,
            ...
            <div>
              {events.map(event => {
                return isHistoryCreateEvent(event) ? (
                  ...
                  isHistoryLinkEvent(event) && (
                    <div className='mcs-fields-list-item'>{this.renderUniqueLinkEvent(event)}</div>
                  )
                );
              })}
            </div>
        ...
        <Row
          style={{ padding: '15px 0px 0px', fontWeight: 'bold' }}
          className='timed-footer text-left'
        >
          <span>
...
export default compose<Props, HistoryEventCardProps>(injectIntl, withRouter)(HistoryEventCard);
