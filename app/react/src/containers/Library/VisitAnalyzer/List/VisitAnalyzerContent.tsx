...
class VisitAnalyzerContent extends Component<RouteComponentProps<RouterProps> & InjectedIntlProps, VisitAnalyzerContentState> {
  ...
  fetchVisitAnalyzer = (organisationId: string, filter: Filters) => {
    this.setState({ loading: true }, () => {
      ...
      VisitAnalyzerService.getVisitAnalyzers(organisationId, options)
  ...
  onClickArchive = (visitAnalyzer: VisitAnalyzer) => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        this.archiveVisitAnalyzer(visitAnalyzer.id)
        .then(() => {
  ...
  onClickEdit = (visitAnalyzer: VisitAnalyzer) => {
    ...
    history.push(`/v2/o/${organisationId}/library/visit_analyzers/${visitAnalyzer.id}/edit`);
  }
  ...
  render() {
    ...
    const actions = {
      ...
      resetList: this.resetVisitAnalyzer,
    };
      ...
      dataColumnsDefinition: [
        {
          ...
          render: (text: string, record: VisitAnalyzer) => (
            <Link
              ...
              to={`/v2/o/${organisationId}/library/visit_analyzers/${record.id}/edit`}
            >{text}
            </Link>
          ),
        },
