...
class CatalogContent extends React.Component<Props, CatalogContentState> {
  ...
  fetchCatalog = (datamatId: string) => {
    const getSelectedCatalogId = (catalogs: CatalogRessource[]) => {
      return this.state.catalogs.selectedId
    ...
    this._libraryCatalogService
      .getCatalogs(datamatId)
      .then(res => res.data)
      .then(catalogs => {
        const a = getSelectedCatalogId(catalogs);
        if (a !== null) {
          this.setState({
            catalogs: { loading: false, items: catalogs, selectedId: a },
          });
          return this.fetchInitialCategories(datamatId, a);
        } else {
          this.setState({
            catalogs: { loading: false, items: [] },
            categories: { loading: false, items: [] },
          });
        }
      })
  ...
  fetchInitialCategories = (datamartId: string, catalogToken: string) => {
    this._libraryCatalogService
      .getCatalogMainCategories(datamartId, catalogToken, {
        depth: 0,
        first_result: 0,
        max_results: 500,
      })
  ...
  fetchSubCategories = (
    ...
    this.setState(
      ...
      () => {
        this._libraryCatalogService
          .getCatalogSubCategories(datamartId, catalogToken, categoryId, {
            depth: 0,
            first_result: 0,
            max_results: 500,
          })
          ...
          .then(categories =>
            this.setState({
              categories: { items: categories, loading: false },
            }),
          )
          .catch(err => this.handleError(err));
      },
  ...
  generateCatalogSelect = () => {
    return this.state.catalogs.items.length ? (
      <Select
        defaultValue={this.state.catalogs.selectedId}
        style={{ minWidth: 120 }}
        onChange={this.handleCatalogChange}
      >
        {this.state.catalogs.items.map(catalog => {
          return (
            <Option key={catalog.token} value={catalog.token}>
              {catalog.token}
            </Option>
          );
        })}
          ...
          return (
            <Breadcrumb.Item key={item.category_id}>
              {i === this.state.path.length - 1 ? (
                ...
                <ButtonStyleless onClick={onCategoryClick(item)}>
                  {item.category_id}
                </ButtonStyleless>
              )}
            </Breadcrumb.Item>
  ...
  renderEmbededView = (record: Category) => {
    ...
    return record.hasItems ? (
      <CatalogItemTable records={record.subItems} />
    ) : (
      <div>{intl.formatMessage(messages.noProduct)}</div>
    );
  };
    ...
    const handleOnRow = (record: Category) => ({
      onClick: () => {
        if (record.hasSubCategory) {
          this.setState(
            {
              path: [...this.state.path, record],
              categories: { loading: true, items: this.state.categories.items },
            },
            () => {
              if (this.state.catalogs.selectedId) {
                this.fetchSubCategories(
