    ...
    return (
      <Row gutter={24}>
        ...
        <Col span={properties.$image_link ? 18 : 24}>
          <div>
            <Tag>Item Id</Tag>: <code>{record.item_id}</code>
          </div>
            ...
            switch (i) {
              case '$link':
                return (
                  <div key={i}>
                    <Tag>{i}</Tag>:{' '}
                    <a href={properties[i]} target='_blank'>
                      {properties[i]}
                    </a>
                  </div>
                );
...
export default compose<Props, CatalogItemTableProps>(injectIntl)(CatalogItemTable);
