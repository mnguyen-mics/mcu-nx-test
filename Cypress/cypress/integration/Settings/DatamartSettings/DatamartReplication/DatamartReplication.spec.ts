...
describe('Datamart Replication Feature Tests', () => {
    ...
    cy.readFile('cypress/fixtures/init_infos.json').then(data => {
      ...
      cy.get('.mcs-content-container')
        ...
        .within(() => {
          ...
          cy.get('.mcs-dropdown-actions').first().contains('Edit').click();
        });
      cy.get('#name').clear().type(editedName);
      cy.get('form').submit();
      cy.contains('Replications').click();
      cy.get('.mcs-content-container').eq(2).should('contain', editedName);
    });
  });
  ...
  it("a datamart replication can't be unpaused if the credentials are wrong", () => {
    cy.readFile('cypress/fixtures/init_infos.json').then(data => {
      ...
      cy.get('form').submit();
      cy.contains('Replications').click();
      cy.get('button.mcs-table-switch').first().click({ force: true });
  ...
  it("when an initial synchro is running we can't create another one", () => {
    cy.readFile('cypress/fixtures/init_infos.json').then(data => {
      ...
      cy.wait(1000);
      cy.get('.mcs-replicationNewExecution_button').click();
      cy.get('.ant-modal-content').contains('New Execution').click();
      cy.wait(1000);
      cy.get('.mcs-replicationNewExecution_button').click();
      cy.wait(1000);
      cy.get('.ant-modal-content').contains('New Execution').click();
