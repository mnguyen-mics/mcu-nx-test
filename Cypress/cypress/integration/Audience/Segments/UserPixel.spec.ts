...
it('Should test the UserPixel Forms', () => {
	cy.readFile('cypress/fixtures/init_infos.json').then((data) => {
		const segmentName: string =
			'Test Audience Segment Form - Test User Pixel Creation Form'
		...
		// Extract the id
		cy.url().should('match', /.*segments\/.*\/edit/)
		cy.url().then((url) => {
			const segmentId: number = parseInt(
				url.substring(
					url.indexOf('segments') + 9,
					url.indexOf('/', url.indexOf('segments') + 9)
				),
				10
			)
			cy.contains('Save').click({ force: true })
			...
			cy.request({
				...
				expect(responseAdd.body.data.datamart_id).to.eq(
					data.datamartId.toString()
				)
				expect(responseAdd.body.data.organisation_id).to.eq(
					data.organisationId.toString()
				)
				expect(responseAdd.body.data.short_description).to.eq(
					segmentDescription
				)
				...
				expect(responseAdd.body.data.subtype).to.eq('USER_PIXEL')
				...
				// Save the new segment
                cy.contains('Save').click({ force: true })
                // Wait before checking the changes
                cy.wait(1000)
				cy.request({
					url: `${Cypress.env('apiDomain')}/v1/audience_segments/${segmentId}`,
					method: 'GET',
                    ...
                    expect(responseEdit.body.data.datamart_id).to.eq(
                        data.datamartId.toString()
                    )
                    expect(responseEdit.body.data.organisation_id).to.eq(
                        data.organisationId.toString()
                    )
                    expect(responseEdit.body.data.short_description).to.eq(
                        segmentDescription+' -edited'
                    )
                    ...
                    expect(responseEdit.body.data.subtype).to.eq('USER_PIXEL')
