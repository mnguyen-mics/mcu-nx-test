describe('This test should check that the audience feature forms are working properly', () => {
  ...
  it('Should test the audience feature forms', () => {
    cy.readFile('cypress/fixtures/init_infos.json').then(data => {
      ...
      cy.get('.mcs-audienceFeature_edit_form_query_builder > textarea')
        .clear({ force: true })
        .type('select {id} from UserPoint where creation_date = $test', {
          force: true,
      ...
      cy.get('.mcs-tableActions_dropdown')
        .first()
        .click();
      cy.get('.mcs-tableActions_settings\\.datamart\\.audienceFeatures\\.edit')
        .first()
        .click();
      ...
      cy.get('.mcs-audienceFeature_edit_form_query_builder > textarea')
        .clear({ force: true })
        .type(
          'select {id} from UserPoint where creation_date > $test & creation_date < $test2',
          {
            force: true,
            parseSpecialCharSequences: false,
          },
      ...
      cy.contains('Audience Builder').click();
      cy.get('.mcs-audienceFeature_table')
        .should('contain', 'creation_date > $test & creation_date < $test2')
        .and('contain', 'Test Audience Feature Form - Edit');
      cy.get('.mcs-tableActions_dropdown')
        .first()
        .click();
      cy.get(
        '.mcs-tableActions_settings\\.datamart\\.audienceFeatures\\.delete',
      )
        .first()
        .click();
    });
